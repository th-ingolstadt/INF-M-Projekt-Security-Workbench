...
function usage {
	...
	echo "env: DOS_WEB_RES_PATH has to be set and point to the location of the web application resources. This directory will be copied to the root folder of the web server."
	exit 1
}

# Check if environment variable DOS_WEB_RES_PATH is set
if [ -z "$DOS_WEB_RES_PATH" ]; then
	echo "Environment variable DOS_WEB_RES_PATH not set!"
	usage
	exit 1
else
	if [ -d "$DOS_WEB_RES_PATH" ]; then
		DOS_WEB_RES_ABSOLUTE_PATH=$(realpath $DOS_WEB_RES_PATH)
		status=$?
		if [ $status -ne 0 ]; then
			echo "Absolute path to environment variable value DOS_WEB_RES_PATH=$WEB_RES_PATH could not be created!"
			exit 1
		fi
	else
		echo "The path of the environment variable DOS_WEB_RES_PATH=$DOS_WEB_RES_PATH could not be found!"
		exit 1
	fi
fi
...
if [ "$1" == "start" ]; then
	...
	echo "Starting DOS docker container ..."
	
	docker run \
		-v $DOS_WEB_RES_ABSOLUTE_PATH:$WEB_SERVER_DST_PATH \
		-p 80:80 \
		--name $CONTAINER_NAME \
		-d $IMAGE_NAME
	status=$?
	...
	echo "... DOS docker container started successfully."
