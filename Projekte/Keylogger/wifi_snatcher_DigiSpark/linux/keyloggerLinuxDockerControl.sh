...
CONTAINER_NAME=keyloggerlinuxcontainer
IMAGE_NAME=keyloggerlinuximage
...
WEB_RES_PATH=$LINUX_KEYLOGGER_WEB_RES_PATH

# usage function
function usage {
	echo "usage: $0 [start|stop|status]"
	echo "env: LINUX_KEYLOGGER_WEB_RES_PATH has to be set and point to the location of the web application resources. This directory will be copied to root folder of the web server."
	echo "Arguments:"
...
function checkEnv {
	if [ -z "$WEB_RES_PATH" ]; then
		echo "Environment variable LINUX_KEYLOGGER_WEB_RES_PATH not set!"
		usage
		exit 1
	else 
		if [ -d "$WEB_RES_PATH" ]; then
			...
			status=$?
			if [ $status -ne 0 ]; then
				echo "Absolute path to environment variable value LINUX_KEYLOGGER_WEB_RES_PATH=$WEB_RES_PATH could not be created!"
				exit 1
			fi
		else
			echo "The path of the environment variable LINUX_KEYLOGGER_WEB_RES_PATH=$WEB_RES_PATH could not be found!"
			exit 1
		fi
	fi
}
